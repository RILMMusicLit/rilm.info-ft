...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
    ...
    <title>RAFT title-entry checks</title>
    ...
    <pattern id="metadata.checks">
        ...
        <rule context="tei:idno[@type='accession-id']">
            <report role="error"
                test="not(following-sibling::tei:idno/@type='Aptara-temp-id' or preceding-sibling::tei:idno/@type='Aptara-temp-id') and not(text())"
        ...
        <rule context="tei:language/@ident">
            ...
            <assert role="error" test="string()"> Language code is missing the required attribute. </assert>
        </rule>
        <!-- new title-type checks below, added 2012/02 key -->
        <rule context="tei:idno[@type='temp-id']">
            <report role="error" test="(count(following-sibling::tei:title[not(*) and normalize-space()])>1) and (count(following-sibling::tei:title[@type='title_entry'])=1)">A new article record should include a true title or a title entry, but not both.</report>
        </rule>
        <rule context="tei:title[@type='title_entry']">
            <assert role="error" test="(ancestor::tei:bibl[@type='periodical-backcover'] | ancestor::tei:bibl[@type='periodical-frontcover'] | ancestor::tei:bibl[@type='advertisement'] | ancestor::tei:bibl[@type='course-list'] | ancestor::tei:bibl[@type='news'] | ancestor::tei:bibl[@type='periodical-colloquy'] | ancestor::tei:bibl[@type='periodical-correspondence'] | ancestor::tei:bibl[@type='periodical-editorial'] | ancestor::tei:bibl[@type='periodical-frontmatter'] | ancestor::tei:bibl[@type='periodical-backmatter'])">This bibl @type normally has a true title, not a title entry.</assert>
            <assert role="error" test="contains (text(), 'Back cover') or contains (text(), 'Front cover') or contains (text(), 'Advertisement') or contains (text(), 'Course list') or contains (text(), 'News') or contains (text(), 'Discussion') or contains (text(), 'Letters') or contains (text(), 'Editorial') or contains (text(), 'Front matter') or contains (text(), 'Publications list') or contains (text(), 'Back matter') or contains (text(), 'Contributors') or contains (text(), 'Society membership') or contains (text(), 'Index')">This title entry contains an invalid string.</assert>
        </rule>
        <!-- Algorithmic checks -->
    ...
    <pattern id="formatting.checks">
        ...
        <rule context="tei:nameLink">
            <assert role="warning"
                test="normalize-space(lower-case(.)) = 'von' or
                ...
                normalize-space(lower-case(.)) = 'dos'"
                > Check contents in nameLink </assert>
        </rule>
    </pattern>
    <pattern id="charset.checks">
        <rule context="//text()">
            <assert role="error" test=". = normalize-unicode(., 'NFC')"> Content is not
                unicode-normalized. </assert>
    ...
    <pattern id="reference.checks">
        ...
        <rule context="tei:title[@type='article']">
            <report role="warning" test="text()[starts-with(upper-case(.),'ART')]">The title element
                should not contain "article" or an abbreviation of it.</report>
        </rule>
        <rule context="tei:pubPlace">
            <report role="warning"
        ...
        <rule abstract="true" id="monograph">
            <report role="error" test="tei:biblScope/@unit='pp'"> The monograph citation should not
                contain page numbers. </report>
        </rule>
        ...
        <rule context="tei:title[@type='collection']">
            ...
            <report role="error" test="following-sibling::tei:biblScope[@unit='sd']"> This is either
                a periodical, or should not have a biblScope/@unit='sd'. </report>
            <report role="error"
        ...
        <rule abstract="true" id="dissertation-cardinality">
            <report role="warning" test="ancestor::tei:list and tei:bibl/@type=''"> The dissertation
                citation for <value-of select=".."/> should be tagged as @diss-doc or @diss-nondoc.
            </report>
        </rule>
        <rule context="tei:title[@type='article']">
...
>>>>>>> aa77c09d1776dac07d95084e6c6891095a808dc2
